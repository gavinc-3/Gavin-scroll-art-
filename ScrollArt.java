...
public class ScrollArt {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        while (true) {
            ...
            int mousePosX = random.nextInt(screenWidth - mousew);
            int mousePosY = random.nextInt(heightterminal - mouseh);
            int fishPosX = random.nextInt(screenWidth - fishw);
            int fishPosY = random.nextInt(heightterminal - fishH);

            for (int x = 0; x < mousew; x++) {
                for (int y = 0; y < mouseh; y++) {
                    if (mouseArt[x][y] != ' ') {
                        screen[mousePosX + x][mousePosY + y] = mouseArt[x][y];
                    }
                }
            }
            ...
            for (int x = 0; x < fishw; x++) {
                for (int y = 0; y < fishH; y++) {
                    if (fishArt[x][y] != ' ') {
                        screen[fishPosX + x][fishPosY + y] = fishArt[x][y];
                    }
                }
            }
